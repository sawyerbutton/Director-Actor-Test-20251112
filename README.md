...
**1.1 引入量化质量评估体系**
- **当前问题**：虽然系统能运行，但缺少量化指标来衡量 TCC 识别的准确性
- **解决方案**：
  - 建立 Golden Dataset 标注系统（人工标注的标准答案）
  - 实现自动化评估指标（Precision、Recall、F1-score）
  - 定期回归测试，跟踪模型质量变化
- **预期收益**：提升系统可信度，量化改进效果

**1.2 优化 Stage 2 的排序算法**
- **当前问题**：A/B/C-line 排序主要依赖 LLM 主观判断
- **解决方案**：
  - 引入量化评分模型（戏份占比、角色数量、冲突强度等）
  - 混合 LLM + 规则引擎，提升排序一致性
  - 实现多轮评审机制（类似 AlphaGo 的 Policy + Value 网络）
- **预期收益**：提高排序准确性和一致性

**1.3 增强 Stage 3 的修改能力**
- **当前问题**：Modifier 主要做删除/移动操作，缺少创造性修改
- **解决方案**：
  - 支持场景合并/拆分建议
  - 生成新场景草稿来填补 setup-payoff 断链
  - 提供多方案对比（让编剧选择不同的修改策略）
- **预期收益**：从"问题诊断"升级为"解决方案生成"
...
- **当前问题**：每次只能分析一个剧本文件
- **解决方案**：
  - 支持整季剧本批量分析（10-20 集一起跑）
  - 实现跨集对比功能（对比不同集的 TCC 分布）
  - 添加项目管理界面（保存历史分析、版本对比）
- **预期收益**：适应实际影视制作流程

**2.2 交互式编辑和实时反馈**
- **当前问题**：分析结果是静态的，无法即时修改
- **解决方案**：
  - 实现 Web 编辑器（在线修改场景，实时重新分析）
  - 添加 What-If 模拟（"如果删除这个场景会怎样？"）
  - 支持协作标注（多人标注 TCC，投票确认）
- **预期收益**：从"分析工具"升级为"创作助手"

**2.3 智能推荐和学习功能**
- **解决方案**：
  - 基于历史数据，推荐类似剧本的最佳实践
  - 实现个性化提示（"您的剧本通常在 Stage 2 会出现这种问题"）
  - 添加剧本模板库（不同类型剧本的典型结构）
- **预期收益**：从工具变为"智能顾问"
...
**3.1 性能优化和成本控制**
- **当前问题**：LLM 调用成本可能较高，尤其是 TXT 增强解析
- **解决方案**：
  - 实现智能缓存（相似场景复用分析结果）
  - 引入小模型预筛选（用便宜模型先过滤，疑难案例才用大模型）
...
- **解决方案**：
  - 建立剧本分析知识库（优秀案例、常见错误）
  - 开放社区版本（让编剧上传匿名剧本，共同改进模型）
  - 发布学术论文（验证系统有效性，提升影响力）
- **预期收益**：构建生态，形成网络效应

#### 💡 推荐优先级

如果要选 3 个最有价值的方向，我们建议：

1. **质量评估体系（方向 1.1）**：这是系统可信度的基础
2. **批量处理和项目管理（方向 2.1）**：大幅提升实用性
3. **智能缓存和成本优化（方向 3.1）**：确保系统可持续运行
...
| **Stage 3** | 结构修正 | ✅ 可用 | 100% |
| **TXT Parser** | 文本剧本解析 + LLM增强 | ✅ 可用 | 100% |
| **Web UI** | 上传、预览、分析界面 | ✅ 可用 | 100% |
...
### ✅ 已解决的问题（最后更新：2025-11-14）

#### Session 7 修复（2025-11-13）
1. ✅ **TCC镜像重叠** - 已通过自动合并解决
...
3. ✅ **Web UI WebSocket序列化错误** - 已通过数据结构重构解决
   - ~~问题~~：WebSocket无法发送包含Script对象的进度更新
   - ✅ **解决**：重构为只发送可序列化的摘要数据（`src/web/app.py:408-441`）
   - 结果：实时进度更新正常工作

4. ✅ **Job状态API序列化错误** - 已通过创建序列化副本解决
   - ~~问题~~：`/api/job/{job_id}` 返回包含Script对象导致500错误
   - ✅ **解决**：创建过滤后的可序列化副本（`src/web/app.py:373-405`）
   - 结果：API正常返回job状态

5. ✅ **前端JavaScript数据结构不匹配** - 已通过修改前端代码解决
   - ~~问题~~：前端期望flat字段（density_score），后端返回nested字段（reasoning.setup_payoff_density）
   - ✅ **解决**：修改formatRankingCard()使用optional chaining访问nested字段（`static/js/results.js:252-278`）
   - 结果：结果页面正常显示，无JavaScript错误

#### Session 8 UX优化与Bug修复（2025-11-14）
6. ✅ **Stage 3 issue_id 验证错误** - 已通过字段归一化解决
   - ~~问题~~：LLM返回 `ISS_001_corrected`、`ISS_002_fixed` 等不符合 `^ISS_\d{3}$` 正则的值
   - ✅ **解决**：添加 `normalize_issue_id()` validator自动提取ISS_XXX模式（`prompts/schemas.py:260-273`）
   - 结果：第一次尝试即成功，无需重试，修正内容统计准确

7. ✅ **Mermaid可视化不显示** - 已通过API升级解决
   - ~~问题~~：可视化标签页只显示代码，不渲染图表
   - ✅ **解决**：
     - 更新为Mermaid 10+ API：`mermaid.render()` 代替废弃的 `mermaid.init()`（`static/js/results.js:348-382`）
     - 修改初始化配置：`startOnLoad: false` 允许动态渲染（`templates/results.html:5-16`）
   - 结果：正确显示彩色TCC关系图（A线黄色、B线蓝色、C线灰色）

8. ✅ **TXT解析event loop错误** - 已通过异步改造解决
   - ~~问题~~：`RuntimeError: no running event loop` 在TXT上传时出现
   - ✅ **解决**：
     - 将 `LLMEnhancedParser.parse()` 改为async方法（`src/parser/llm_enhancer.py:82-125`）
