...
### Session 9: Gemini 2.5 Flash 集成

#### 问题背景
- **问题**: 使用 DeepSeek 分析大脚本 (12+ 场景) 时, Stage 3 (Modifier) 输出 JSON 被截断
- **原因**: DeepSeek 的 max_tokens 限制为 16K, 无法满足大脚本的输出需求
- **测试文件**: `百妖1.txt` (12 场景)
- **错误信息**: `Invalid JSON: EOF while parsing a string at line 530`

#### 解决方案
集成 Google Gemini 2.5 Flash 作为替代 LLM 提供商:
- **上下文窗口**: 1M tokens (vs DeepSeek 64K)
- **输出限制**: 65K tokens (vs DeepSeek 16K)
- **完全解决** Stage 3 JSON 截断问题

#### 完成工作

1. ✅ **核心代码修改**
   - `src/pipeline.py:288-303`: 添加 Gemini provider 支持
   - `src/web/app.py:122-131`: 传递 default_provider 到模板
   - `templates/index.html:94-110`: 添加 Gemini 选项到提供商选择器
   - `src/ab_testing.py`: 更新提供商列表

2. ✅ **依赖和配置**
   - `requirements.txt`: 添加 `langchain-google-genai>=2.0.0`
   - `docker-compose.yml`: 添加 `GOOGLE_API_KEY` 环境变量
   - `.env.example`: 添加 Gemini API Key 配置说明

3. ✅ **测试脚本**
   - 新建 `test_gemini_api.py` (90 行)
   - 用于业务测试前验证 API 连通性
   - 测试模型可用性、API Key、基本推理

4. ✅ **文档更新**
   - 新建 `ref/gemini-integration.md` (13KB, 525 行)
   - 更新 `CLAUDE.md` (v2.5.0)
   - 新建 `PROJECT_STATUS.md` (331 行)

#### 调试历程

| 尝试 | 模型名称 | 结果 | 问题 |
|------|---------|------|------|
| 1 | `gemini-3-pro-preview` | 失败 | 配额超限 (limit: 0) |
| 2 | `gemini-2.0-flash-thinking-exp` | 失败 | 模型不存在 |
| 3 | `gemini-2.5-flash-latest` | 失败 | 模型名称错误 |
| 4 | `gemini-2.5-flash` | **成功** | 正确的模型名称 |

**关键学习**: Gemini 模型名称必须精确匹配官方文档, 不能随意添加后缀如 `-latest`
...
**大脚本测试** (`百妖1.txt`, 12 场景):
```
Stage 1 (Discoverer): 19.66s → 3 TCCs 识别
Stage 2 (Auditor): 27.40s → A/B/C 线排名
Stage 3 (Modifier): 59.84s → 6 问题修复
总计: 106.90s, 0 错误, 0 重试
```

**单场景测试** (`蓝1 第三版.txt`, 1 场景):
```
预期失败: 最少需要 2 场景
错误: List should have at least 1 item after validation, not 0
重试: 3 次后失败 (正确行为)
验证: 错误处理机制工作正常
```
...
```
修改: 9 个文件 (+133 行, -33 行)
新增: 2 个文件 (test_gemini_api.py, ref/gemini-integration.md)
总计: 11 个文件, +747 行代码
```

#### Git 提交
- **Commit**: `d956340`
- **Message**: `feat: integrate Google Gemini 2.5 Flash for large script support (v2.5.0)`

#### 性能对比

| 指标 | DeepSeek | Gemini 2.5 Flash | 胜者 |
|------|----------|------------------|------|
| 速度 | 更快 (~15s Stage 1) | 较慢 (~20s Stage 1) | DeepSeek |
| 可靠性 | ❌ 大脚本失败 | ✅ 全部成功 | **Gemini** |
| 成本 | $0.14/1M input | $0.075/1M input | **Gemini** |
| 输出限制 | 16K tokens | 65K tokens | **Gemini** |
| 上下文窗口 | 64K tokens | 1M tokens | **Gemini** |

#### 使用建议
- **小脚本** (<10 场景): 使用 DeepSeek (更快)
- **大脚本** (10+ 场景): 使用 Gemini (更可靠)

#### 遗留问题
1. ⚠️ 单场景剧本无法分析 (需修改 prompt 或添加前置检查)
2. ⚠️ Gemini 配额可能受限 (需监控使用量)
